...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'create': {
        const code = makeCode();
        const room = { clients: new Set([ws]), masterWs: ws, lastTrack: null };
        rooms.set(code, room);
        ws.room = code;
        ws.isMaster = true;
      ...
      case 'join': {
        ...
        // catch the new listener up on whatever is currently loaded
        if (room.lastTrack) {
          ws.send(JSON.stringify({
            ...room.lastTrack,
            type: 'control',
            action: 'load',
            targetServerTime: Date.now() + 300,
          }));
        }
        sendPeerCount(room);
        break;
      ...
      case 'control': {
        ...
        if (msg.action === 'load') {
          // pass through track metadata so listeners can render it too
          base.title = msg.title || '';
          base.artist = msg.artist || '';
          base.artwork = msg.artwork || '';
          base.duration = msg.duration || 0;
          room.lastTrack = { url: base.url, position: base.position, title: base.title, artist: base.artist, artwork: base.artwork, duration: base.duration };
        ...
        if (room.lastTrack && (msg.action === 'play' || msg.action === 'seek')) {
          room.lastTrack.position = base.position;
        }
